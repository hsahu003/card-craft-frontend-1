    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "EditorPage.useEffect": ()=>{
            ...
            const renderTextHandles = {
                "EditorPage.useEffect.renderTextHandles": (tid)=>{
                    selectedTextId = tid;
                    // Remove any existing handle groups
                    Array.from(svgEl.querySelectorAll('[data-text-handles="1"]')).forEach({
                        "EditorPage.useEffect.renderTextHandles": (g)=>{
                            g.parentNode?.removeChild(g);
                        }
                    }["EditorPage.useEffect.renderTextHandles"]);
                    if (!tid) return;
                    ...
                    corners.forEach({
                        "EditorPage.useEffect.renderTextHandles": ({ key, cx, cy, cursor })=>{
            ...
            const onMouseMove = {
                "EditorPage.useEffect.onMouseMove": (e)=>{
                    ...
                    if (drag.type === "resize") {
                        ...
                        const applyFontSize = {
                            "EditorPage.useEffect.onMouseMove.applyFontSize": (el)=>{
                                el.setAttribute("font-size", String(newFont));
                                el.style.fontSize = String(newFont) + "px";
                                const tspans = Array.from(el.querySelectorAll("tspan"));
                                tspans.forEach({
                                    "EditorPage.useEffect.onMouseMove.applyFontSize": (t)=>{
                                ...
                                }["EditorPage.useEffect.onMouseMove.applyFontSize"]);
                            }
                        }["EditorPage.useEffect.onMouseMove.applyFontSize"];
                        applyFontSize(docEl);
                        applyFontSize(liveText);
                        const updatePosition = {
                            "EditorPage.useEffect.onMouseMove.updatePosition": (el)=>{
                                let bb = null;
                                try {
                                    const b = el.getBBox?.();
                                    if (b && b.width > 0 && b.height > 0) {
                                ...
                                const anchorKey = ({
                                    "EditorPage.useEffect.onMouseMove.updatePosition.anchorKey": ()=>{
                                ...
                                if (tspans.length) {
                                    tspans.forEach({
                                        "EditorPage.useEffect.onMouseMove.updatePosition": (t)=>{
                                    ...
                                    }["EditorPage.useEffect.onMouseMove.updatePosition"]);
                                } else {
